...
export async function download() {
  ...
  const initializePuppeteerNode = require('puppeteer-core/lib/cjs/puppeteer/initialize-node').initializePuppeteerNode;
  const revisions = require('puppeteer-core/lib/cjs/puppeteer/revisions.js').PUPPETEER_REVISIONS[product];
  const revisionsInfo = initializePuppeteerNode().createBrowserFetcher(product).revisionInfo(revisions);

  if (revisionsInfo.local) {
    return;
  }
  ...
  const {
    downloadBrowser,
    logPolitely,
  } = require('puppeteer-core/lib/cjs/puppeteer/node/install');
