...
import { existsSync, ensureDir, emptyDir, readFileSync, realpathSync, unlinkSync, writeFile, rmdir } from 'fs-extra';
...
export class App {
    ...
    public async initialize() {
        ...
        this.program.command('export [bookUrl]')
            ...
            .option('--dist [distDir]', '导出位置, 默认位置为当前目录先 docs 文件夹')
            .description('导出掘金小册')
    ...
    private html2md = async (content: string, imgSaveDir: string) => {

        let ret: string = _html2md(content, {
            skipTags: ['div', 'html', 'body', 'dl', 'dd', 'dt', 'img']
        });

        const srcMap = new Map();
        let countId = 0;

        ret = ret.replace(this.imgSrcReg, (_: any, s1: string) => {
            countId++;
            srcMap.set(countId, s1);
            return `<img src="__PLACEHOLDER_${countId}__" />`
        });
    ...
    private setCache = async (data: any) => {
        ...
        await writeFile(this.cacheTokenFilePath, JSON.stringify({
            _t: Date.now(),
            ...data,
        }));
    }
    ...
    private exportBook = async (bookUrl: string, command: Command) => {
        ...
        let saveDir = join(command.distDir || join(this.cwd, 'docs'), bookId);
        if(existsSync(saveDir)) {
            await emptyDir(saveDir);
            await rmdir(saveDir);
        }
...
${await this.html2md(bookInfo.data.introduction.content, saveDir)}
        `;
        ...
        for (const sectionInfo of bookInfo.data.sections) {
            ...
            const fileContent = `
${await this.html2md(content, saveDir)}
